...
| `questions_display` | object | null | Self-check questions display configuration (opt-in, `enabled: false` by default) |
...
### Questions Display Configuration

Self-check questions are generated by DeepSeek during topic extraction. Opt-in: `enabled: false` by default for backward compatibility.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `enabled` | boolean | false | Enable questions display (opt-in) |
| `format` | enum | "numbered_list" | Format: `"numbered_list"`, `"bullet_list"`, `"dash_list"`, `"comma_separated"`, `"inline"` |
| `max_count` | integer(1-20) | null | Max number of questions to show (default: all). Extraction produces `transcription.questions_count` (default 3) |
| `min_length` | integer(0-500) | null | Min question length in chars (default: 0 = no filter) |
| `max_length` | integer(10-1000) | null | Max question length in chars |
| `prefix` | string(0-200) | null | Prefix before questions list |
...
| `transcription.questions_count` | integer | 3 | Number of self-check questions for DeepSeek to generate |
...
| `questions_display` | object | Questions display config (common, opt-in) |
...
- `{summary}` - Summary
- `{questions}` - Self-check questions (formatted when `questions_display.enabled: true`)
