...
В `config/fireworks_creds.json` должны быть `api_key` и `account_id` (остальные параметры ASR — через env `FIREWORKS_*`, см. `FireworksSettings` в `config/settings.py`). Шаблон: `config/examples/fireworks_creds.json.example`.
