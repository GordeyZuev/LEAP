...
Public URL: `{origin}/c/{slug}`. Slug is **globally unique**, 5–64 chars, `a-z0-9`, hyphen, and underscore. It can be changed (PATCH); the old URL **404**s (no redirect). **Enable / Disable** — Disable keeps the slug; delete frees it. There is no UUID token and no Rotate.
