...
- **CORS via env (`scripts/vm-init.sh` .env.static)** — добавлен `SERVER_CORS_ORIGINS=https://leap-platform.ru` (default был `["*"]` — не блокирующее security-issue из-за Bearer-токенов, но best-practice).
