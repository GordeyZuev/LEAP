...
class ZoomAPI:
    ...
    async def get_current_user(self) -> dict[str, Any]:
        """GET /users/me — cheapest authenticated call, used to verify credentials.

        Raises ``ZoomAuthenticationError`` when Zoom rejects the credentials, so callers
        can tell a dead app or secret from Zoom being temporarily unreachable.
        """
        ...
        try:
            ...
            raise ZoomRequestError(f"Network error: {type(e).__name__}") from e

        if response.status_code in (401, 403):
            raise ZoomAuthenticationError(f"Zoom rejected the credentials ({response.status_code})")
        if response.status_code >= 400:
            raise ZoomResponseError(f"API error: {response.status_code} - {response.text[:200]}")
