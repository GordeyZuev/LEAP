...
def _created_at_window(
    *,
    ...
    if days is not None:
        return datetime.now(UTC) - timedelta(days=days), datetime.now(UTC)
    raise ValueError(f"{what} requires days or from_dt/to_dt")
...
class ShareEventRepository:
    ...
    async def daily_aggregates(
        ...
        *,
        ...
        since, until = _created_at_window(days=days, from_dt=from_dt, to_dt=to_dt, what="daily_aggregates")
    ...
    async def downloads_by_type(
        ...
        *,
        ...
        return await self.downloads_by_type_for_recordings([recording_id], days=days, from_dt=from_dt, to_dt=to_dt)

    async def downloads_by_type_for_recordings(
        self,
        ...
        *,
        ...
        if not recording_ids:
            return {}
        since, until = _created_at_window(days=days, from_dt=from_dt, to_dt=to_dt, what="downloads_by_type")
        type_col = func.coalesce(ShareAccessEventModel.artifact_type, "file")
        result = await self.session.execute(
            select(type_col, func.count())
            .where(
