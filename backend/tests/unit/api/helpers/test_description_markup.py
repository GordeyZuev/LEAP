...
class TestPlaylistDescriptionJinja:
    def test_context_counts_and_items(self) -> None:
        rec_a = SimpleNamespace(id=1, display_name="Intro", final_duration=65.0, duration=65.0)
        rec_b = SimpleNamespace(id=2, display_name="Joins", final_duration=125.0, duration=125.0)
    ...
    def test_render_substitutes(self) -> None:
        rec = SimpleNamespace(id=1, display_name="A", final_duration=60.0, duration=60.0)
    ...
    def test_items_use_rendered_titles_when_provided(self) -> None:
        rec = SimpleNamespace(id=9, display_name="Source name", final_duration=60.0, duration=60.0)
        playlist = SimpleNamespace(id=1, items=[SimpleNamespace(position=0, recording=rec)])
