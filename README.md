...
**Версия:** `v0.10.8.2` (September 2026) · **Статус:** In Active Development • Beta
...
**Новое в `v0.10.8.2`** — **Описания плейлистов и Overview**: жирный, курсив, подчёркивание, зачёркивание и ссылка; подстановки вроде числа видео не меняют своё оформление. На публичной странице курса сами подставляются число видео, суммарная длительность и список пунктов. На YouTube и VK уходит обычный текст. Редактор показывает, как текст записан; **Public look** — как у зрителя (формат на одной строке). **Плеер:** J и L ±10 с, стрелки ±5 с, C — субтитры в шестерёнке, клавиша «вопрос» — список сочетаний; на панели текущее время и длительность.
...
- **yt-dlp: устойчивость** — retry при rate-limit от YouTube; дружелюбные сообщения об ошибках авторизации/ограничений; общие опции (`get_ydl_opts`) применяются при извлечении форматов.

**Новое в `v0.10.5.0`** — большое обновление веб-интерфейса плюс рабочая система квот и расширенный Admin API.

Веб-интерфейс — дизайн-система, тёмная тема, мобильная адаптация:
- **Тёмная тема** — переключатель Light / Dark / System в *Settings*; выбор сохраняется и применяется без вспышки при загрузке. По умолчанию следует системной теме.
- **Мобильная адаптация** — на телефоне сайдбар стал выезжающим drawer'ом с гамбургером в верхней панели; таблицы, конфиги платформ, лендинг и модалки корректно адаптируются под узкие экраны.
- **Единая дизайн-система** — все цвета переведены на семантические токены (готовность к теме и брендингу); добавлены переиспользуемые `PageHeader`, `EmptyState`, `ErrorState`, `Skeleton` — консистентные состояния загрузки/пустоты/ошибки на всех страницах.
- **Живой отклик** — анимации появления модалок/дропдаунов/тостов (с уважением к `prefers-reduced-motion`); подтверждающие тосты на действиях (запуск/пауза/сброс/переименование/загрузка), кнопка повтора при ошибке.
- **Доступность** — `aria-live` на ошибках форм, `aria-expanded` на сворачиваемых секциях, корректные роли у тостов и переключателей.

Бэкенд — квоты, история действий и Admin API:
- **Квоты считают по-настоящему** — объём хранилища берётся из S3/MinIO (а не с локального диска), счётчики записей, транскрибаций, обработок и загрузок реально инкрементируются; лимит одновременных задач считается из флага `on_air` (drift-free).
- **Hard limits** — превышение лимитов на хранилище, транскрибацию и обработку возвращает `429` (в т.ч. на уровне Celery-задач, чтобы auto-run/bulk не обходили лимит).
- **Все feature-флаги работают** — `403` при отключении прав на транскрибацию, обработку, темы/субтитры, загрузку, удаление, экспорт и управление credentials. Все 8 флагов управляются админом.
- **Видно использование** — `GET /me/quota` показывает текущее потребление (записи, хранилище, транскрибации, обработки, загрузки) и лимиты.
...
- **Email-верификация** — новый аккаунт требует подтверждения адреса; письмо с кнопкой приходит сразу после регистрации.
- **Восстановление пароля** — ссылка «Forgot password?» на странице входа; одноразовый токен, TTL 1 ч, инвалидация всех сессий после смены пароля.
