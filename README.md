...
- **Copy endpoints** — дублирование шаблонов (`POST /templates/{id}/copy`), пресетов (`POST /presets/{id}/copy`) и заданий автоматизации (`POST /automations/{id}/copy`) одной кнопкой; имена генерируются автоматически («Copy of …»).
- **Исправление WebM/VP8 pipeline** — тримминг WebM-видео (VP8/VP9 + Vorbis/Opus) больше не падает из-за несовместимого контейнера: выходной формат (`.mp4` / `.webm` / `.mkv`) определяется по реальным кодекам через ffprobe перед обрезкой.
- **Устойчивость тримминга** — перед запуском FFmpeg проверяется наличие видео- и аудиопотоков; потоки маппируются явно только если существуют; ошибки ffprobe не ронят весь пайплайн.
- **Улучшенный yt-dlp** — логируется фактически выбранное качество и кодек; предупреждение, если запрошен MP4 но yt-dlp выдал VP8/VP9 (нет H.264-стрима для выбранного качества).
- **Фронтенд v0.1** — полный редизайн всех страниц (записи, шаблоны, пресеты, автоматизации, настройки, источники, credentials); новые компоненты `platform-fields` и `thumbnail-picker`; централизованные константы поллинга и пагинации.
- **Валидация `folder_path`** — Yandex Disk source автоматически добавляет ведущий `/` к пути папки если он отсутствует.
- **`InputSourceListItem`** — список источников теперь включает `description` и `config` (раньше исключались из list-view).
...
**Новое в v0.9.6.4:**
- **Cookies для yt-dlp** — `YTDLP_COOKIES_FILE` / `YTDLP_COOKIES_FROM_BROWSER` (обход блокировок YouTube и стабильный доступ)
- **Английский на всём пайплайне** — `language: en` в конфиге: ASR → темы → LLM → субтитры
