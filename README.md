...
**Версия:** `v0.11.0.2` (September 2026) · **Статус:** Beta · **Возраст:** 12+
...
**Новое в `v0.11.0.2`** – **МТС Линк и VK:** если площадка просит подождать (2 запроса/с на ключ МТС, 3/с на токен VK), LEAP ждёт секунды и повторяет — лекция не падает после короткого всплеска и не ждёт 10 минут в общей куче. Исчерпанный 429 на подготовке МТС паркует запись, а не помечает failed. **Обрезка:** пауза в середине лекции (кофе-брейк) больше не считается концом видео. **Безопасность:** исходящие URL только http(s) без private IP; смена email с паролем и повторной верификацией; refresh/reset/verify как SHA-256 (миграция **055** — старые письма перезапросить).

**Новое в `v0.11.0.1`** – **Нажатия:** кнопки, табы, сегменты и карточки слегка сжимаются и чуть темнеют при нажатии, без подскока. **Admin** в сайдбаре рядом с Automation, из Settings — ссылка. **Обработка:** пока пайплайн идёт, на статусе спиннер, а не «Ready». **Автоматизации:** окно last-N дней включительно в часовом поясе джоба, Preview без синка по умолчанию, живой статус RUNNING, списки из пяти строк с «Show all».
